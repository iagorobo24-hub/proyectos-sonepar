...
export default function DashboardIncidencias() {
  ...
  const generarDiagnostico = async (inc) => {
    ...
    try {
      ...
      const { text } = await callAnthropicAI({ model: "anthropic/claude-3.5-haiku", max_tokens: 1000, system: PROMPT_DIAGNOSTICO(inc), messages: [{ role: "user", content: "Diagnostica esta incidencia." }] });
