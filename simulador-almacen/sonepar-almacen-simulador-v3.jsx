...
const LogoSonepar = ({ size = 26 }) => (
  <svg width={size * 3.2} height={size} viewBox="0 0 120 38" fill="none">
    ...
    <ellipse cx="16" cy="19" rx="14" ry="7.5" stroke={C.blanco}    strokeWidth="2.2" fill="none" transform="rotate(20 16 19)" />
    <text x="34" y="25" fontFamily="Helvetica Neue,Arial,sans-serif" fontWeight="700" fontSize="17" fill={C.blanco} letterSpacing="0.5">sonepar</text>
  </svg>
);
...
// estructura: { id, etapa, titulo, descripcion, opciones[{texto, correcto, feedback}] }
const INCIDENCIAS = [
  {
    id: "INC-01", etapa: 0,
...
const getSemaforo = (t, est) => {
  if (!est) return null;
  const pct = (t / est) * 100;
  if (pct <= 100) return { label: "Dentro del estándar", color: C.verde,    bg: C.verdeSuave };
  if (pct <= 150) return { label: "Lento",               color: C.amarillo, bg: C.amarilloS  };
  return              { label: "Muy lento",              color: C.rojo,     bg: C.rojoSuave  };
};

// Puntuación: base 100, -5 por cada incidencia fallada, -10 por etapa Muy Lento, -5 por etapa Lento
...
export default function SimuladorAlmacen() {
  // Pantallas: perfil | onboarding | selector | simulacion | resultado
  const [pantalla, setPantalla]         = useState("perfil");
  const [operario, setOperario]         = useState({ nombre: "", turno: "Mañana", area: "Almacén" });
  const [modoSim, setModoSim]           = useState("entrenamiento"); // entrenamiento | evaluacion
  ...
  // Incidencias
  const [incActiva, setIncActiva]       = useState(null);   // incidencia que se muestra ahora
  const [incResueltas, setIncResueltas] = useState([]);     // { titulo, correcto, feedback }
  const [incPendientes, setIncPendientes] = useState([]);   // ids de incidencias del turno no usadas
  const [feedbackInc, setFeedbackInc]   = useState(null);   // respuesta a mostrar post-elección

  const intervalRef    = useRef(null);
  const inicioEtapaRef = useRef(null);

  // Cargar perfil + historial
  useEffect(() => {
    try {
  ...
  // ── Sortear incidencias para el pedido (2-3 incidencias por sesión) ─────────
  const sortearIncidencias = () => {
    const pool = [...INCIDENCIAS];
    // 1 por etapa 0-1, 1 por etapa 2-3, 1 por etapa 4 (probabilidad 50%)
    const elegidas = [];
    [0, 1].forEach(etapa => {
  ...
  // ── Comprobar si hay incidencia para la etapa actual ────────────────────────
  const comprobarIncidencia = (etapaId) => {
    ...
    if (candidatas.length) {
      ...
      if (intervalRef.current) clearInterval(intervalRef.current); // pausa cronómetro
    }
  };
  ...
  // ── Confirmar feedback y continuar ──────────────────────────────────────────
  const continuarTrasFeedback = () => {
    setFeedbackInc(null);
    // Reinicia cronómetro desde 0 en la etapa actual
    inicioEtapaRef.current = Date.now();
    setTiempoEtapa(0);
  };
  ...
  const avanzarEtapa = () => {
    ...
    if (etapaActual < ETAPAS.length - 1) {
      ...
      // Comprobar incidencia al entrar en la siguiente etapa
  ...
  // Semáforo en tiempo real del cronómetro
  ...
  const dificultadColor = { "Básico": C.verde, "Intermedio": C.amarillo, "Avanzado": C.rojo, "Manual": C.textoTer };
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: C.fondo, fontFamily: "system-ui,Segoe UI,sans-serif", color: C.texto }}>
        ...
        {/* ── Header ────────────────────────────────────────────────────────── */}
        <div style={{ background: C.azulOscuro, padding: "0 32px", display: "flex", alignItems: "center", gap: "20px", height: "56px" }}>
          <LogoSonepar size={24} />
          <div style={{ width: "1px", height: "28px", background: "rgba(255,255,255,0.2)" }} />
          <span style={{ color: C.blanco, fontSize: "13px", fontWeight: "500" }}>Simulador de Almacén</span>
          <span style={{ color: "rgba(255,255,255,0.4)", fontSize: "11px" }}>v3.0</span>
          {operario.nombre && <span style={{ color: "rgba(255,255,255,0.6)", fontSize: "12px", marginLeft: "4px" }}>· {operario.nombre}</span>}
          <div style={{ flex: 1 }} />
          <button onClick={() => setMostrarHistorial(!mostrarHistorial)} style={{ padding: "6px 14px", fontSize: "12px", background: mostrarHistorial ? "rgba(255,255,255,0.15)" : "transparent", color: mostrarHistorial ? C.blanco : "rgba(255,255,255,0.6)", border: "none", borderRadius: "6px", cursor: "pointer", fontFamily: "system-ui,Segoe UI,sans-serif" }}>
            Historial ({historial.length})
          </button>
          {pantalla !== "perfil" && pantalla !== "onboarding" && pantalla !== "selector" && (
            <button onClick={resetear} style={{ ...btnS, fontSize: "11px", padding: "5px 12px", background: "transparent", color: "rgba(255,255,255,0.7)", borderColor: "rgba(255,255,255,0.3)" }}>
              ← Nuevo pedido
            </button>
          )}
        </div>
              ...
              {historial.slice(0, 8).map((h, i) => (
                <div key={i} style={{ background: C.fondo, border: `1px solid ${C.borde}`, borderRadius: "8px", padding: "10px 14px", minWidth: "200px" }}>
                  ...
                  <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                    ...
                    <span style={{ fontSize: "13px", fontWeight: "700", color: h.puntuacion >= 80 ? C.verde : h.puntuacion >= 60 ? C.amarillo : C.rojo }}>{h.puntuacion}pts</span>
          ...
          {pantalla === "simulacion" && pedidoActivo && (
            <div className="fade-in" style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "20px" }}>
              ...
              <div>
                ...
                <div style={{ background: C.blanco, border: `1px solid ${C.borde}`, borderRadius: "10px", padding: "14px 18px", marginBottom: "18px", borderLeft: `4px solid ${C.azulOscuro}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  {incResueltas.length > 0 && (
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ fontSize: "14px", fontWeight: "700", color: incResueltas.filter(r => r.correcto).length === incResueltas.length ? C.verde : C.amarillo }}>
                    ...
                    return (
                      <div key={i} style={{ flex: 1, textAlign: "center", position: "relative" }}>
                        ...
                        <div style={{ position: "relative", zIndex: 1 }}>
                          <div style={{ width: "38px", height: "38px", borderRadius: "50%", margin: "0 auto 6px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", background: completada ? (sem?.color || C.verde) : activa ? C.azulOscuro : C.fondo, color: completada || activa ? C.blanco : C.textoTer, fontWeight: "700", border: activa ? `3px solid ${C.azulClaro}` : `2px solid ${completada ? "transparent" : C.borde}`, transition: "all 0.2s" }}>
                            {completada ? "✓" : e.icono}
                          </div>
                          <div style={{ fontSize: "10px", fontWeight: activa ? "700" : "500", color: activa ? C.texto : C.textoTer }}>{e.nombre}</div>
                          {completada && <div style={{ fontSize: "10px", color: sem?.color || C.verde, fontWeight: "600" }}>{fmtT(tiempos[i])}</div>}
                        </div>
                      </div>
                    );
                  })}
                ...
                {/* ── Feedback tras incidencia ─────────────────────────── */}
                {feedbackInc && (
                  <div className="slide-down" style={{ background: feedbackInc.correcto ? C.verdeSuave : C.amarilloS, border: `2px solid ${feedbackInc.correcto ? C.verde : C.amarillo}`, borderRadius: "10px", padding: "20px", marginBottom: "18px" }}>
                    <div style={{ display: "flex", gap: "10px", alignItems: "center", marginBottom: "10px" }}>
                      <span style={{ fontSize: "22px" }}>{feedbackInc.correcto ? "✅" : "⚠️"}</span>
                      <div style={{ fontSize: "14px", fontWeight: "700", color: feedbackInc.correcto ? C.verde : C.amarillo }}>
                  ...
                  <div style={{ background: C.blanco, border: `2px solid ${C.azulOscuro}`, borderRadius: "10px", padding: "22px" }}>
                    ...
                    <button onClick={avanzarEtapa} style={{ ...btnP, width: "100%", padding: "14px", fontSize: "14px" }}>
                      {etapaActual < ETAPAS.length - 1
                        ? `Completar ${ETAPAS[etapaActual].nombre} → `
                        : "Completar ciclo ✓"}
                    </button>
                  </div>
                )}
          ...
          {pantalla === "resultado" && pedidoActivo && tiempos.length > 0 && (
            <div className="fade-in" style={{ maxWidth: "820px", margin: "0 auto" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "20px" }}>
                {[
                  { label: "PUNTUACIÓN", valor: `${puntuacionActual}`, unidad: "/ 100", color: puntuacionActual >= 80 ? C.verde : puntuacionActual >= 60 ? C.amarillo : C.rojo },
                  { label: "TIEMPO TOTAL", valor: fmtT(tiempos.reduce((a,b) => a+b,0)), unidad: "", color: C.azulOscuro },
                  { label: "INCIDENCIAS", valor: `${incResueltas.filter(r => r.correcto).length}/${incResueltas.length}`, unidad: "correctas", color: incResueltas.every(r => r.correcto) ? C.verde : C.amarillo },
                ...
                {ETAPAS.map((e, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "160px 90px 90px 80px 1fr", padding: "11px 16px", borderBottom: i < 4 ? `1px solid ${C.fondo}` : "none", alignItems: "center", background: i % 2 === 0 ? C.blanco : C.fondo }}>
                      ...
                      <div style={{ fontWeight: "700", color: desv > 0 ? C.rojo : C.verde }}>
                        {desv > 0 ? "+" : ""}{desv}%
                      </div>
                ...
                <div style={{ background: C.blanco, border: `1px solid ${C.borde}`, borderRadius: "10px", padding: "18px", marginBottom: "16px" }}>
                  ...
                  <div style={{ display: "grid", gap: "8px" }}>
                    {incResueltas.map((r, i) => (
                      <div key={i} style={{ display: "flex", gap: "10px", alignItems: "flex-start", padding: "10px 12px", background: r.correcto ? C.verdeSuave : C.rojoSuave, borderRadius: "8px" }}>
                        <span style={{ fontSize: "16px", flexShrink: 0 }}>{r.correcto ? "✅" : "❌"}</span>
                        <div>
                          <div style={{ fontSize: "12px", fontWeight: "700", color: r.correcto ? C.verde : C.rojo, marginBottom: "2px" }}>{r.titulo}</div>
